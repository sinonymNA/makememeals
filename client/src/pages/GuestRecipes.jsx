...
function PrintableRecipeCard({ meal, id }) {
  ...
  const steps       = typeof meal.steps === 'string' ? JSON.parse(meal.steps) : (meal.steps || []);

  return (
    <div id={id} className="recipe-card-print mx-5" style={{ padding: '24px', background: '#FEFEF6' }}>
      <div className="flex items-center gap-2 pb-3 mb-3" style={{ borderBottom: '2px solid var(--border)' }}>
        <span className="text-[13px] font-extrabold" style={{ color: 'var(--accent)' }}>🍽️ MAKE ME MEALS</span>
      </div>
      <div className="text-[40px] text-center mb-2">{meal.emoji}</div>
      <h2 className="text-[22px] font-black text-center leading-tight mb-1" style={{ color: 'var(--text)' }}>{meal.name}</h2>
      ...
      <div className="text-[11px] font-extrabold uppercase tracking-widest mb-2" style={{ color: 'var(--accent)' }}>WHAT YOU NEED</div>
      ...
      <div style={{ borderBottom: '1.5px solid var(--border)', marginBottom: '14px' }} />
      <div className="text-[11px] font-extrabold uppercase tracking-widest mb-2" style={{ color: 'var(--accent)' }}>HOW TO MAKE IT</div>
      <div className="flex flex-col gap-2 mb-4">
        {steps.map((step, i) => (
          <div key={i} className="flex gap-2 text-[13px]" style={{ color: 'var(--text)' }}>
            <span className="font-black flex-shrink-0" style={{ color: 'var(--accent)', minWidth: '16px' }}>{i + 1}.</span>
...
export default function GuestRecipes() {
    ...
    <div className="app-shell" style={{ background: 'var(--bg)' }}>
      ...
      <div className="page-pad pt-6 pb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[13px] font-semibold" style={{ color: 'var(--text-mid)' }}>{currentIdx + 1} of {meals.length}</p>
        </div>
      </div>

      <div className="flex items-center justify-between px-5 mb-3">
        <button className="w-10 h-10 raised-sm flex items-center justify-center" onClick={() => setCurrentIdx(i => Math.max(0, i - 1))} disabled={currentIdx === 0} style={{ borderRadius: '12px', opacity: currentIdx === 0 ? 0.3 : 1 }}>
          <ChevronLeft size={18} style={{ color: 'var(--text-mid)' }} />
        </button>
        <div className="flex gap-1.5">
          {meals.map((_, i) => (
            <button key={i} className="w-2 h-2 rounded-full transition-all" style={{ background: i === currentIdx ? 'var(--accent)' : 'var(--border)' }} onClick={() => setCurrentIdx(i)} />
          ))}
        </div>
        <button className="w-10 h-10 raised-sm flex items-center justify-center" onClick={() => setCurrentIdx(i => Math.min(meals.length - 1, i + 1))} disabled={currentIdx === meals.length - 1} style={{ borderRadius: '12px', opacity: currentIdx === meals.length - 1 ? 0.3 : 1 }}>
          <ChevronRight size={18} style={{ color: 'var(--text-mid)' }} />
        </button>
      </div>

      <PrintableRecipeCard meal={meal} id={`recipe-card-${currentIdx}`} />

      <div className="page-pad flex gap-3 mt-5 pb-10">
        <button className="pill-button ghost flex-1 justify-center text-[14px]" onClick={() => exportAsImage(`recipe-card-${currentIdx}`, meal.name)}>
          <Image size={15} /> Save Image
        </button>
        <button className="pill-button flex-1 justify-center text-[14px]" onClick={() => exportAsPDF(meal)}>
